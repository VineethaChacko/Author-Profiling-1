...
def get_classification(Matrix, languagevarietyList, Language, modelFolder):
    ...
    for i in range (0,10):
        ...
        languagevarietyModel = RandomForestClassifier(n_estimators =100,max_depth=2, random_state=0)
    ...
    start = timer()
    languagevarietyModel = RandomForestClassifier(max_depth=2, random_state=0)
    languagevarietyModel.fit(X_train, y_train)
    end = timer()
    print(end - start)
